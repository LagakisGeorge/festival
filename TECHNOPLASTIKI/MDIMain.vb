...
Public Class MDIMain
    ...
    Private Sub MDIMain_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        ...
        ExecuteSQLQuery("SELECT COUNT(*) AS N FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_NAME  = 'KINEMP'", R)    'On Error Resume Next

        If R(0)(0) = 0 Then
            Dim CSQL As String
            CSQL = "CREATE TABLE KINEMP ([ID] [int] IDENTITY(1,1) NOT NULL,[TEMAXIA] [real] NULL,[KOD] [int] NOT NULL,[HME] [datetime] NULL,[PARTIDA] [BIGint] NULL ) "

            ExecuteSQLQuery(CSQL, R)
        End If
    ...
    Private Sub cmdERGASIES_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles cmdExcel.Click
        ...
        ExecuteSQLQuery("SELECT TOP 1 BASTIM_GID FROM MEM", SQ)

        Dim CCV As String = Str(SQ(0)(0))
        mergates.Label1.Text = "SELECT KOD AS [ΚΩΔ.ΕΙΔΟΥΣ],PROM AS[ΠΡΟΜΗΘΕΥΤΗΣ],ATIM,HME,round(YPOL,1) AS [ΥΠΟΛ],POSO AS [ΑΡΧ.ΠΟΣΟΤΗΤΑ],' ' as [.],ID  FROM TIMS WHERE YEAR(HME)>" + CCV + "  ORDER BY KOD,HME "  ' where YPOL>0
    ...
    Sub ylika2(ByVal mn1 As String, ByVal per As String)
        Dim frm As New ergates  ' form2 
        'Dim Mn1 As String = "3"
        frm.Label1.Text = "SELECT ONO AS [Ονομα ],KOD AS [ΚΩΔ],N1 AS [ΚΑΤΗΓ],BAROS AS [ΒΑΡΟΣ],MEMO,C1,PIECESPERPALLET AS [TEM/ΠΑΛΕΤ],ID  FROM YLIKA WHERE N1=" + mn1 + " ORDER BY KOD "
    ...
    Private Sub Button5_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button5.Click
        ...
        Dim frm As New ergates  ' form2 
        'Dim Mn1 As String = "3"
        frm.Label1.Text = "select top 400 PARTIDA AS [ΠΑΡΤΙΔΑ],HME as [ΗΜΕΡ.ΠΑΡΑΓ],P.KOD as [ΠΡΟΪΟΝ],ONO AS [ΠΕΡΙΓΡΑΦΗ ΕΙΔΟΥΣ],TEMAXIA AS [ΠΑΡΑΧΘΕΝΤΑ],TEMAXIA-YPOL AS [ΠΩΛΗΘΕΝΤΑ],YPOL AS [ΥΠΟΛΟΙΠΟ],P.N2 AS [ΠΑΛΕΤΑ],P.ID  FROM PARTIDES P INNER JOIN YLIKA Y ON P.KOD=Y.KOD  ORDER BY HME DESC "  ' PARTIDA DESC "where GETDATE()-HME<30
    ...
    Private Sub Button7_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button7.Click
        Dim per = "Λίστα Τιμολογίων Πώλησης με Παρτίδες(με Control+σειρά έχω πολλαπλή επιλογή(multi select)"
        Dim kod = "00"
        ...
        'MultiSelect property to True and SelectionMode to FullRowSelect
        mergates.GridView1.MultiSelect = True
        mergates.GridView1.SelectionMode = DataGridViewSelectionMode.FullRowSelect
    ...
    Private Sub Button9_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles Button9.Click
        ...
        Dim frm As New ergates  ' form2 
        'Dim Mn1 As String = "3"
        frm.Label1.Text = "select KOD,EPO,AFM ,ID  FROM PEL  WHERE EIDOS='e' ORDER BY EPO "
    ...
    Private Sub SupplierProfileToolStripMenuItem_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles SupplierProfileToolStripMenuItem.Click
        ...
        ExecuteSQLQuery("SELECT COUNT(*) AS N FROM INFORMATION_SCHEMA.TABLES WHERE TABLE_TYPE = 'BASE TABLE' AND TABLE_NAME  = 'PALETES'", R)    'On Error Resume Next

        If R(0)(0) = 0 Then
            ExecuteSQLQuery("CREATE TABLE PALETES ( PALET BIGINT,KOD VARCHAR(15),ONO VARCHAR(35),PARTIDA VARCHAR(10),POSO BIGINT,ID int identity(1,1) NOT NULL )", R)
        End If
    ...
    Private Sub TSHoldLeft_ItemClicked(ByVal sender As System.Object, ByVal e As System.Windows.Forms.ToolStripItemClickedEventArgs) Handles TSHoldLeft.ItemClicked
    ...
    Sub paletes()
        ...
        ' Dim frm As New ergates  ' form2 
        ' frm.Label1.Text = "SELECT PALET AS [ΠΑΛ],P.KOD AS [ΚΩΔ],Y.ONO,P.PARTIDA,P.POSO,P.ID  FROM PALETES P  INNER JOIN YLIKA Y ON P.KOD=Y.KOD   ORDER BY PALET "

        Dim ANS As Integer
        ANS = MsgBox("Αθροισμα ανά Κωδικό(Ναι)/Ανα κωδικό-παρτίδα=Οχι", MsgBoxStyle.YesNo)

        If ANS = vbNo Then
            frm.Label1.Text = "SELECT top 400 PALET AS [ΠΑΛ],P.KOD AS [ΚΩΔ],CONVERT(CHAR(23),DATE,126) AS HMEP,Y.ONO,P.PARTIDA,SUM(P.POSO) AS [TEM],ATIMPOL AS [ΤΙΜ.ΠΩΛ],ATIMPIS AS [ΕΠΙΣΤΡ],P.KOD AS [IDD]  FROM PALETES P  INNER JOIN YLIKA Y ON P.KOD=Y.KOD  GROUP BY PALET,P.KOD,Y.ONO,P.PARTIDA,ATIMPOL ,ATIMPIS ,DATE  ORDER BY PALET desc"
        Else
            frm.Label1.Text = "SELECT top 400 PALET AS [ΠΑΛ],P.KOD AS [ΚΩΔ],CONVERT(CHAR(23),DATE,126) AS HMEP,Y.ONO,SUM(P.POSO) AS [TEM],P.KOD AS [ΚΩΔΙΚΟΣ],ATIMPOL AS [ΤΙΜ.ΠΩΛ],ATIMPIS AS [ΕΠΙΣΤΡ],DATE,P.KOD AS [IDD]  FROM PALETES P  INNER JOIN YLIKA Y ON P.KOD=Y.KOD  GROUP BY PALET,P.KOD,Y.ONO ,ATIMPOL ,ATIMPIS ,DATE  ORDER BY PALET desc"
        End If
        ...
        frm.widths(3) = 300
        ' frm.n1.Text = mn1
        frm.Alignments(3) = DataGridViewContentAlignment.MiddleLeft ' MiddleCenter
        frm.Alignments(4) = DataGridViewContentAlignment.MiddleLeft
    ...
    Private Sub cmdExcel_MouseDown(ByVal sender As Object, ByVal e As System.Windows.Forms.MouseEventArgs) Handles cmdExcel.MouseDown
        ...
        If e.Button = Windows.Forms.MouseButtons.Right Then
            ...
            C = InputBox("ΑΠΟ ΠΟΙΟ ΕΤΟΣ ΚΑΙ ΜΕΤΑ ΤΑ ΥΠΟΛΟΙΠΑ;", "", Str(SQ(0)(0)))
            ExecuteSQLQuery("UPDATE MEM SET  BASTIM_GID=" + C, SQ)
